...
function expandPatterns(input: string, cwd = process.cwd()): string[] {
...
function countHeadings(text: string, sections: string[]): number {
  return sections.filter((section) => new RegExp(`^#{1,3}\\s+${section}\\b`, "mi").test(text)).length;
}

function inferKind(path: string, text: string): DocumentKind | null {
  ...
  const scores = (Object.entries(requiredByKind) as [DocumentKind, string[]][])
